...
class NewsAdminForm(GoogleDriveAdminForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for name in ("photo1", "photo2", "photo3"):
            self.fields[name].help_text = "Вставьте ссылку на изображение или загрузите файл ниже в Google Drive."
    ...
    def clean(self):
        cleaned_data = super().clean()
        uploader = None
        for field_name, upload_name in (("photo1", "upload_file"), ("photo2", "upload_file_2"), ("photo3", "upload_file_3")):
            upload = cleaned_data.get(upload_name)
            if upload:
                try:
                    uploader = uploader or GoogleDriveUploader()
                    cleaned_data[field_name] = uploader.upload(upload, filename=upload.name).web_url
                except ImproperlyConfigured as exc:
                    raise ValidationError(str(exc))
        return cleaned_data
